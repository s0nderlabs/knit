...
export function PlanDisplay({
  ...
  return (
    ...
    >
      ...
      <div className="relative ml-1 border-l border-dashed border-border pl-5 mt-3">
        {modules.map((mod, i) => (
          <motion.div
            key={mod.name}
          ...
          >
            ...
            <div className="absolute -left-5 top-[9px] h-[1px] w-4 border-t border-dashed border-border" />
            {/* Node dot */}
            <div className="absolute -left-[22.5px] top-[5px] h-2 w-2 rounded-full border-2 border-border bg-surface" />
      ...
      <div className="mt-5 flex items-center gap-2">
        <button
          onClick={onBuild}
          className="rounded-xl bg-ink px-4 py-2 text-sm font-medium text-white transition-colors hover:bg-ink/85"
        >
          Build
        </button>
        <button
